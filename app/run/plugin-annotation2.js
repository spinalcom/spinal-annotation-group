        ...
        if(selected != null) {
          for (let j = 0; j < selected.listModel.length; j++) {
            ...
            content += `<md-list-item class="noright _annotation" id="an_${note.id.get()}">
                ...
                <md-button class="i_btn show${note.id.get()}" id='e_${note.id.get()}' aria-label="view" ng-click="exec_function('view','${selected.id.get()}','${note.id.get()}')" show="${note.display.get()}">
